...
2.  **Queuing (Producer):** Instead of locking the database, the request is serialized and pushed to a **RabbitMQ** queue (`orders_queue`). The user receives an immediate acknowledgement.
3.  **Processing (Worker):** A background **Goroutine (Consumer)** picks up messages from the queue sequentially.
...
│   ├── models            # GORM Structs (Product, Order)
